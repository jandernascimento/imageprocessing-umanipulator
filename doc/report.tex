...
\section{Filters}
	...
	\subsection{Fundamental}
		...
		\subsubsection{Gradient}
			...
			short description of edge detection concept. Technically, edge is a part of image where a sharp change is occured in the brightness of the pixels. More informally, edges are the boundaries of
			...
			Mathematically, gradient is the first derivation of the image. In other words, it is a vector with a certain magnitude and direction. The magnitude of the gradient provides information about strength
			of the edge. Additionally, the direction of the gradient is always prependicular to the direction of the edge. Clearly, the gradient filter could be
			...
			In this application the Prewitt kernel is used for gradient-based edge detection.
			As it is mentioned previously, gradient approach is based on the first derivation of the input. Based on the properties of
			the foist derivation, the edges of a givent input could be seen near the picks in the first derivation curve. Accordingly, a threshold should be
			...
			as an edge. As a result, we may need to apply a localization method (for example, using Laplacian filter) to find zero
			crossing points and separate actual edges from other non-edge parts.
